...
TELEGRAM_TOKEN = "8886129807:AAGkQ5aJs8g1m42xyub0WCiwa9EbHO-izrI"
CHAT_ID = "8632875840"
    ...
    for s in sources:
        try:
            ...
            if r.status_code == 200:
                ...
                for t in titles[:2]:
                    if len(t) > 10:
                        news_items.append({"source": s["name"], "title": t.strip(), "time": time.strftime("%H:%M:%S")})
        except:
            pass
    return news_items
...
def analyze(news):
    ...
    return {"total": len(news), "positive": p, "negative": n}
...
send_telegram("💀 Solvatron has shut down.")
''')
